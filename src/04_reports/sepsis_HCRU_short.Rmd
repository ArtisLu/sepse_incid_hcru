...
load(file = "data/proc/match_object.RData")
...
max_lookback <- bind_rows(
  ambul %>% filter(pid %in% cohort_matched$pid) %>% select(pid, date = sakuma_datums),
  stac %>% filter(pid %in% cohort_matched$pid) %>% select(pid, date = date1),
  komp_med %>% filter(pid %in% cohort_matched$pid) %>% select(pid, date = datums)
) %>% 
  arrange(date) %>% 
...
cohort_matched %>% 
  ...
  select(nave, fup_time = time, lookback_time, hosp_ilgums, vecums, dzimums, mi:aids, charlson, hosp_gads, cohort) %>% 
  tbl_summary(
    by = cohort,
